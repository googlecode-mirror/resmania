...
class RM_Plugin_Config_Parser {
    ...
    function getConfig($filepath)
    {
        ...
        if ($configArray === false) {
            ...
            throw new RM_Exception($translate->_('RM.Plugin.Config.Parser', 'InvalidIniFileFormat'));
        }

        $this->_config = new RM_Plugin_Config();
        ...
        foreach ($configArray as $section => $values) {
            ...
            if (in_array($parseMethodName, $methods) == false ) {
                throw new RM_Exception($translate->_('RM.Plugin.Config.Parser', 'WrongIniFileSectionName').':'.$section);
            }            
        ...
        $validator = new RM_Plugin_Config_Validator();
        try {
    ...
    private function _parseInformation($values){
        $sectionArray = $this->_config->information;
    ...
    private function _parseDependency($value){
        ...
        $dependency = array(
            'child_type' => RM_Plugin_Row::DEPENDENCY_TYPE,
