...
//calculate new holiday list for current year
			...
			if(ddMatch && format){
				...
				//if(ddMatch.test(fvalue)){
                                if(ddMatch.test && ddMatch.test(fvalue)){
					tiptext = ddText.replace("%0", fvalue);					
					cell.className = " x-date-disabled";
				}
                                if (ddMatch[0]) { //RM changes
                                    for(var ddIndex = 0; ddIndex < ddMatch.length; ddIndex++){
                                        if (ddMatch[ddIndex].dateFormat(format) == fvalue) {
                                            tiptext = ddText.replace("%0", fvalue);
                                            cell.className = " rm-calendar-reserved-days";
                                            break;
                                                    }
                                    }
                                }
