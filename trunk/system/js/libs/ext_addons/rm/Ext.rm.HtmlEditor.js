...
Ext.rm.HtmlEditor_SpecialCharacters = Ext.extend(Ext.util.Observable, {
    ...
    onRender: function(){
        var cmp = this.cmp;
        var btc = this.cmp.getToolbar();
        if (!btc){
            return;
        }
        var btn = btc.addButton({
            iconCls: 'x-edit-char',
            handler: function(){
...
Ext.rm.HtmlEditor = Ext.extend(Ext.form.HtmlEditor, {
    getDocMarkup : function(){
        return '<html><head><style type="text/css">body{border:0;margin:0;padding:3px;height:90%;cursor:text;}</style></head><body></body></html>';
    },
    cleanHtml: function(html) {
        // we don't want html cleanup so just return the html
        return html;
    },
    ctCls: 'rm-htmleditor'
});

Ext.reg('rm_htmleditor', Ext.rm.HtmlEditor);